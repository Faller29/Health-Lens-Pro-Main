...
class _FoodSelectorPageState extends State<FoodSelectorPage> {
  ...
  void showSelectedFoodsBottomSheet(BuildContext context) {
    ...
    List<Map<String, String>> selectedFoodList = [];

    for (var food in itemMacronutrients.keys) {
      for (var serving in itemMacronutrients[food]!.keys) {
        String key = '$food ($serving)';
        if (selectedItems.contains(key)) {
          // Instead of saving a single string, save as separate fields
          selectedFoodList.add({
            'foodName': food, // e.g., Chicken (Chicken Inasal)
            'servingPart': serving // e.g., Breast
          });
          totalCarbs += itemMacronutrients[food]![serving]!['carbs'] ?? 0;
          totalFats += itemMacronutrients[food]![serving]!['fats'] ?? 0;
          totalProteins += itemMacronutrients[food]![serving]!['proteins'] ?? 0;
        }
      }
    }
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (selectedFoodList.isEmpty)
                      ...
                      ...selectedFoodList
                          .map((food) => Text(
                                '${food['foodName']} (${food['servingPart']})',
                                style: GoogleFonts.readexPro(
                                  fontSize: 14.0,
                                  textStyle: const TextStyle(
                                    color: Color.fromARGB(255, 0, 0, 0),
  ...
  Future<void> saveSelectedFoods(
      List<Map<String, String>> selectedFoodList) async {
